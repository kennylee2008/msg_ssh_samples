...
public class MessageDaoImpl extends BaseDao implements MessageDao {
	...
	public void delSendMessage(int sendMessageId) {
		Session session = getSession();
		session.delete(session.load(Message.class, sendMessageId));
	}
	...
	public Message findSendMessageById(int sendMessageId) {
		return (Message)getSession().load(Message.class, sendMessageId);
	}
	
	public int findUnreadedMessages(int receiverId){
		String hql = "select count(*) from ReceiveMessage m where m.receiver.id = ? and m.hasRead = ?";
